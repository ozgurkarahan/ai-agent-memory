...
Skills live in **two parallel locations** that must stay in sync:

- `.github/instructions/{slug}.instructions.md` — for GitHub Copilot CLI (`applyTo: "**"` frontmatter required)
- `memory/workflows/{slug}.md` — canonical plain-markdown procedure (no frontmatter)

When you change one, update the other in the same commit. The bodies should be identical except for the frontmatter block on the `.github/instructions/` copy.
